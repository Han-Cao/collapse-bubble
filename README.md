...
2. When merging genotypes, the phase of genotypes is checked and retained:
    - 0|0 and 1|0: compatible, merge into 1|0
    - 0|1 and 1|0: compatible, merge into 1|1
    - 1|0 and 1|0: incompatible, will not merge if found in any sample
...
**Note**: `merge_duplicates.py` can concatenate any overlapping variants at the same position when `-c position` is used. This method reconstructs the local haplotype, significantly increasing polymorphism, which is not suitable for SV merging. Additionally, it has stricter requirements for the input VCF. Please carefully read the [documentation](docs/merge_duplicates.md) before using it. It is worth noting that `merge_duplicates.py -c position` is included in the [minigraph-cactus](https://github.com/ComparativeGenomicsToolkit/cactus) v2.9.4. If you are interested in generating a VCF that reconstruct overlapping variants as much as possible, please try the latest MC pipeline instead.
...
Major:
- [ ] new SV merging pipeline that concatenates overlapping variants before SV merging, which will improve variant merging for tandem repeats and more compatible with latest MC pipeline.
- [ ] workflow script to run all analysis in the pipeline.

Minor:
- [ ] update AC, AN, AF in the output VCF
- [ ] support ploidy > 2
